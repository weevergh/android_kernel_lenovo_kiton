...
int msm_cpu_pm_enter_sleep(enum msm_pm_sleep_mode mode, bool from_idle)
{
	...
	time = sched_clock();
	if (execute[mode]) {
		//yangjq, 20140113, Add to show suspend clock logs, START
		if (!from_idle && mode == MSM_PM_SLEEP_MODE_POWER_COLLAPSE)
			clock_debug_print_enabled();
		//yangjq, 20140113, Add to show suspend clock logs, END
