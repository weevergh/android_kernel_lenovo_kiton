...
#define ES325_SLEEP_DELAY 5000
#define ES325_SLEEP_DELAY_IN_BOOT 300
...
static unsigned int es325_power_state = 0;
...
static void es325_sleep_request_in_boot(struct es325_priv *es325);

unsigned int es325_presets[3] = {0,0,0};
static int es325_rx_ch_num_to_idx(int ch_num)
{
	int i;
	int idx = -1;
...
static int fw_download(void *arg)
{
    ...
    es325_sleep_request_in_boot(priv);
...
static int es325_sleep(struct es325_priv *es325)
{
    ...
    else 
    {
        ...
        for (i = 0; i < SLEEP_TRIAL; i ++)
        { 
            rc = ES325_BUS_READ(es325, ES325_READ_VE_OFFSET,
                    ES325_READ_VE_WIDTH, msg, 4, 1);
            if (rc < 0) {
                pr_err("%s(): request sleep OK", __func__);
...
static void es325_sleep_request_in_boot(struct es325_priv *es325)
{
    ...
    if (es325_priv.power_state == ES325_POWER_AWAKE)
    {
        schedule_delayed_work(&es325->sleep_work,
                msecs_to_jiffies(ES325_SLEEP_DELAY_IN_BOOT));
...
static int set_preset_coeff_value(unsigned int value,struct es325_priv *es325)
{
    ...
    pr_debug("%s preset value : %d es325_ptr : %p\n",__func__,value,es325);
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
    ...
    unsigned int value;
	struct es325_priv *es325 = &es325_priv;
    int count = ((struct soc_multi_mixer_control *)
					kcontrol->private_value)->count;
    ...
    for (i = 0; i < count ; i++){
        ...
        pr_debug("%s preset value : %d es325_ptr : %p\n",__func__,value,es325);
        ret =  set_preset_coeff_value(value,es325);
       	if (ret < 0) {
			pr_err("%s(): set_preset_coeff_value() err", __func__);
			return ret;
		}
       	es325_presets[i] = value & 0xffff; 
        pr_info("%s() exit: preset[%d] = %d \n", __func__,i,es325_presets[i]);
    }
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
    ...
    for (i=0; i < count ; i++)
        ucontrol->value.integer.value[i] = es325_presets[i];
...
static struct snd_kcontrol_new es325_digital_ext_snd_controls[] = {
    ...
    /*
	SOC_SINGLE_EXT("ES325 Presets1",SND_SOC_NOPM, 0, 65535, 0,
                   es325_preset_get_control_value, es325_preset_put_control_value),
    SOC_SINGLE_EXT("ES325 Presets2",SND_SOC_NOPM, 0, 65535, 0,
                   es325_preset_get_control_value, es325_preset_put_control_value),
    SOC_SINGLE_EXT("ES325 Presets3",SND_SOC_NOPM, 0, 65535, 0,
                   es325_preset_get_control_value, es325_preset_put_control_value)
    */
    SOC_SINGLE_MULTI_EXT("ES325 Presets",SND_SOC_NOPM, 0, 65535, 0, 3,
                         es325_preset_get_control_value, es325_preset_put_control_value),
};
