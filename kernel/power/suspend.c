...
//#define AP_STATE_SMEM_RESERVED_6
	...
	if (!error) {
		*wakeup = pm_wakeup_pending();
		if (!(suspend_test(TEST_CORE) || *wakeup)) {
			//yangjq, 20131225, Inform RPM apps is power collapsed
#ifndef AP_STATE_SMEM_RESERVED_6
			smem_set_reserved(2, 1);
#else
			smem_set_reserved(6, smem_get_reserved(6) | 0x80000000);
#endif //AP_STATE_SMEM_RESERVED_6
			error = suspend_ops->enter(state);
			//yangjq, 20131225, Inform RPM apps is power restored
#ifndef AP_STATE_SMEM_RESERVED_6
