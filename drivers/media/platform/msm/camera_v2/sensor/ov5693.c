...
struct otp_struct {
	...
	int user_data[5];
	int lenc[62];
	int otp_awb_caled; // 1, it is already calculated; 0 , not
...
#define OV5693_RG_Ratio_Typical 343
#define OV5693_BG_Ratio_Typical 328
...
#if 1
{
	...
	int j=0;
	for(j=0;j<62;j++)
		ov5693_debug(OV5693_INFO, "lenc[%d]=0x%x\n",j, (* otp_ptr).lenc[j]);
...
int ov5693_update_otp_wb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(ov5693_otp.otp_awb_caled == 0)
	{
		...
		ov5693_otp.otp_awb_caled = 1;
	}
	ov5693_update_awb_gain(s_ctrl,ov5693_otp.R_gain, ov5693_otp.G_gain, ov5693_otp.B_gain);
	return 0;
}
// call this function after OV5693 initialization
// return value: 0 update success
...
int ov5693_update_otp_lenc(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(ov5693_otp.otp_lenc_readed == 0)
	{
		for(i=1;i<=3;i++) {
		...
		ov5693_otp.otp_lenc_readed =1;
	}
	ov5693_update_lenc(s_ctrl,&ov5693_otp);
	// success
	return 0;
}
...
static int32_t ov5693_platform_probe(struct platform_device *pdev)
{
	...
	ov5693_otp.otp_awb_caled = 0;
	ov5693_otp.otp_lenc_readed =0;
	return rc;
}
