...
int32_t read_eeprom_memory(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	for (j = 0; j < eb_info->num_blocks; j++) {
		CDBG("%s dts info [block num:%d]addr change emap[j].mem.addr=0x%x, slave addr=0x%x\n", 
			__func__,j,emap[j].mem.addr ,e_ctrl->i2c_client.cci_client->sid);		
		while(emap[j].mem.addr > 0xFF)
		{
			CDBG("%s before addr change emap[j].mem.addr=0x%x, slave addr=0x%x\n", 
				__func__,emap[j].mem.addr ,e_ctrl->i2c_client.cci_client->sid);

			emap[j].mem.addr-=0x100;
			e_ctrl->i2c_client.cci_client->sid++;
			CDBG("%s after addr change emap[j].mem.addr=0x%x, slave addr=0x%x\n", 
				__func__,emap[j].mem.addr ,e_ctrl->i2c_client.cci_client->sid);
		}
