	...
	uint16_t gpio_array_size)
{
	...
	if (of_property_read_bool(of_node, "qcom,gpio-img-en-1") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-img-en-1", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-img-en-1 failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-img-en-1 invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_IMG_EN_1] =
			gpio_array[val];
		CDBG("%s qcom,gpio-img-en-1 %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_IMG_EN_1]);
	}
	if (of_property_read_bool(of_node, "qcom,gpio-img-en-2") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-img-en-2", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-img-en-2 failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-img-en-2 invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_IMG_EN_2] =
			gpio_array[val];
		CDBG("%s qcom,gpio-img-en-2 %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_IMG_EN_2]);
	}
	return 0;
...
int32_t msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	for (index = 0; index < power_setting_array->size; index++) {
		...
		case SENSOR_VREG:
#ifdef CONFIG_SND_SOC_ES325
			if(seq_val_reg_dig==1 && power_setting->seq_val==CAM_VDIG)
				break;
#endif
	...
	void __user *argp)
{
	...
	case CFG_SENSOR_OTP_PROC: {
		...
		CDBG("CFG_SENSOR_OTP_PROC rc =%lx\n", rc);
		break;
	}
	default:
		rc = -EFAULT;
