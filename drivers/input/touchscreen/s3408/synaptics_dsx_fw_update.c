...
static int fwu_start_reflash(void)
{
	...
	if(config_id[0] == 4){
		printk(KERN_ALERT "4++++++++\n");
		if(config_id[3] == 5){
		printk(KERN_ALERT "4++++++++++new firmware do not flash\n");
			retval = -EINVAL;
			goto exit;
		}
	}else {
		...
		printk(KERN_ALERT "5+++++++++\n");
			if(config_id[3] == 11){
		printk(KERN_ALERT "5++++++++++new firmware do not flash\n");
				retval = -EINVAL;
				goto exit;
			}

		} else {
		printk(KERN_ALERT "firmware config id = %d\n", config_id[0]);
	}
	}
