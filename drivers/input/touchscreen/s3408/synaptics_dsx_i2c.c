...
static struct regulator *regulator3;
#define VDD_2P8		"8941_l18"
#define VDD_I2C		"8941_lvs1"
#define VDD_1P8		"8941_l12"

static volatile int key_db = 1;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	key_db =(unsigned int)val;
	printk(KERN_DEBUG "key_db value = %ld.\n", val);
	return count;
}
...
extern int hall_ctrl(int val)
{
	int retval;
	unsigned char cal = 0x02;
	unsigned char sen1 = 0x01;
	unsigned char sen2 = 0x00;
	unsigned char pen1[2] = {0x0a, 0x0d};
	unsigned char pen2[2] = {0x0f, 0x14};
	struct device *dev = t_dev;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (likely(extra_data->data15_size)) {
		retval = synaptics_rmi4_i2c_read(rmi4_data,
	...
	if (unlikely(!fingers_to_process)) {
		synaptics_rmi4_free_fingers(rmi4_data);
		return 0;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		dev_dbg(&rmi4_data->i2c_client->dev,
				...
				status);
		printk(KERN_DEBUG "Button %d,status %d.\n",f1a->button_map[button], status);
#ifdef NO_0D_WHILE_2D
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!list_empty(&rmi->support_fn_list)) {
		list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
			if (likely(fhandler->num_of_data_sources)) {
				if (likely(fhandler->intr_mask &
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	if (likely(!rmi4_data->touch_stopped))
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F34:
				...
				printk(" device_config_id 1-2-3-4: %x %x %x %x.\n",config_id[0],
		...
		const struct i2c_device_id *dev_id)
{
	...
	unsigned char sen[2] = {0x0f, 0x14};
	unsigned char sam = 0x02;
	unsigned char key_sen = 0x00;

	       gpio_request(60, "touch-reset");
...
*/
	...
	regulator1 = regulator_get(&rmi4_data->i2c_client->dev, VDD_2P8);
	...
	regulator3 = regulator_get(&rmi4_data->i2c_client->dev, VDD_1P8);
	if(IS_ERR(regulator3)){
		printk(KERN_ERR "failed to get tp regulator\n");
		return -EINVAL;
	} else{
		if(regulator_count_voltages(regulator3) > 0){
			regulator_set_voltage(regulator3, 1800000, 1800000);
			regulator_set_optimum_mode(regulator3, 15000);
		}else
			printk(KERN_ERR"--------------set voltage error");
	}

	regulator2 = regulator_get(&rmi4_data->i2c_client->dev, VDD_I2C);
	if(IS_ERR(regulator2)){
		printk(KERN_ERR "failed to get tp regulator\n");
	} else{
		regulator_count_voltages(regulator2);
		regulator_set_voltage(regulator2, 1800000, 1800000);
		regulator_set_optimum_mode(regulator1, 10000);
	}
	regulator_enable(regulator1);
	regulator_enable(regulator2);
	regulator_enable(regulator3);
