...
static int dcp_current_max = 1500;
static int dcp_current_max_func(const char *s, struct kernel_param *kp)
{
	int ret = 0;
	long current_val = 0;
	struct power_supply	*psy = NULL;
	ret = strict_strtol(s,10, &current_val);
	printk("set dcp charging current max with %d\n",(int)current_val);
	if (ret < 0)
		return ret;	
	psy = power_supply_get_by_name("usb");
	power_supply_set_current_limit(psy, current_val*1000);
	power_supply_changed(psy);
	dcp_current_max = (int)current_val;
	return 0;
}
module_param_call(dcp_current_max,dcp_current_max_func,NULL,NULL,0644);
...
//	power_supply_set_supply_type(psy, power_supply_type);
	...
	dev_info(phy->dev, "Avail curr from USB = %u charger type is %d\n", mA,dotg->charger->chg_type);
	...
	if (dotg->charger->max_power <= 2 && mA > 2) {
		...
		/* Disable charging */
		if (power_supply_set_online(dotg->ac_psy, false))
			goto psy_error;
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_UNDEFINED:
		...
		if (!dotg->ac_psy) {
			dotg->ac_psy = power_supply_get_by_name("ac");

			if (!dotg->ac_psy)
				dev_err(phy->dev,
					 "couldn't get ac power supply\n");
		}
