...
static int dwc3_msm_power_get_property_ac(struct power_supply *psy,
...
static int dwc3_msm_power_set_property_ac(struct power_supply *psy,
				  enum power_supply_property psp,
				  const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		mdwc->ac_online = val->intval;
		break;
	default:
		return -EINVAL;
	}

	power_supply_changed(&mdwc->ac_psy);
	return 0;
}
...
static enum power_supply_property dwc3_msm_pm_power_props_ac[] = {
...
static int __devinit dwc3_msm_probe(struct platform_device *pdev)
{
	...
	if (mdwc->ext_xceiv.otg_capability ||
		...
		mdwc->ac_psy.name = "ac";
		mdwc->ac_psy.type = POWER_SUPPLY_TYPE_MAINS;
		mdwc->ac_psy.supplied_to = dwc3_msm_pm_power_supplied_to;
		mdwc->ac_psy.num_supplicants = ARRAY_SIZE(
						dwc3_msm_pm_power_supplied_to);
		mdwc->ac_psy.properties = dwc3_msm_pm_power_props_ac;
		mdwc->ac_psy.num_properties =
					ARRAY_SIZE(dwc3_msm_pm_power_props_ac);
		mdwc->ac_psy.get_property = dwc3_msm_power_get_property_ac;
		mdwc->ac_psy.set_property = dwc3_msm_power_set_property_ac;

		ret = power_supply_register(&pdev->dev, &mdwc->ac_psy);
		if (ret < 0) {
			dev_err(&pdev->dev,
					"%s:power_supply_register ac failed\n",
						__func__);
			goto disable_hs_ldo;
		}
	}
