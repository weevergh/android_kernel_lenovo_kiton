...
#if LENOVO_KPD
#define KPDBL_ROW_SCAN_FREQ(base)	(base + 0xB0)
#define KPDBL_GLOBAL_ROW_SCAN(base)	(base + 0xB1)
#define KPDBL_ROW_SCAN(base)		(base + 0xB2)
#endif
/**
...
static int __devinit qpnp_kpdbl_init(struct qpnp_led_data *led)
{
	...
	/* program row scan freqency */
	rc = qpnp_led_masked_write(led, KPDBL_ROW_SCAN_FREQ(led->base),
		0x00, 0x03);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
			"row scan frequency write failed(%d)\n", rc);
		return rc;
	}

	/* program global row scan */
	rc = qpnp_led_masked_write(led, KPDBL_GLOBAL_ROW_SCAN(led->base),
		0x80, 0x80);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
			"global row scan write failed(%d)\n", rc);
		return rc;
	}

	/* program row scan for the needed rows */
	rc = qpnp_led_masked_write(led, KPDBL_ROW_SCAN(led->base),
		0x0F, 0x0F);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
			"row scan write failed(%d)\n", rc);
		return rc;
	}
