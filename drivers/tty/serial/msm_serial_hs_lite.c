...
#if 1//def CONFIG_SERIAL_SC6500_WAKE_LOCK
...
#if 1//def CONFIG_SERIAL_SC6500_WAKE_LOCK
	bool host_mdm_wake_lock;
	atomic_t mdm_wake_locked;
	struct timer_list mdm_wake_unlock_timer;
...
#if 1//def CONFIG_SERIAL_SC6500_WAKE_LOCK
#define HOST_MDM_WAKE_LOCK_TIME_OUT (3 * HZ)

static inline struct uart_port * get_port_from_line(unsigned int line);

static ssize_t 
host_mdm_wake_lock_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct uart_port *port = get_port_from_line(pdev->id);
	struct msm_hsl_port *msm_port = UART_TO_MSM(port);
	return sprintf(buf, "%d\n", msm_port->host_mdm_wake_lock);
}

static ssize_t 
host_mdm_wake_lock_store(struct device *dev, struct device_attribute *attr,
	   const char *buf, size_t count)
{
    ...
    state = !!state;
    if (msm_port->host_mdm_wake_lock != state) {
        msm_port->host_mdm_wake_lock = state;
        if (!state) {
            wake_unlock((&msm_port->host_wake_lock));
            if (atomic_cmpxchg(&(msm_port->mdm_wake_locked), 1, 0))
                wake_unlock_mdm();
        }
    }
...
static DEVICE_ATTR(host_mdm_wake_lock, (0600), host_mdm_wake_lock_show, host_mdm_wake_lock_store);

static void host_wake_lock(struct msm_hsl_port *port) {
    if (port->host_mdm_wake_lock) {
        pr_debug("[msm_serial_hs]%s\n", __FUNCTION__);
        wake_lock_timeout(&(port->host_wake_lock), HOST_MDM_WAKE_LOCK_TIME_OUT);
    }
}

static void host_wake_unlock(struct msm_hsl_port *port) {
    if (port->host_mdm_wake_lock) {
        pr_debug("[msm_serial_hs]%s\n", __FUNCTION__);
        wake_unlock((&port->host_wake_lock));
    }
}

static void mdm_wake_lock(struct msm_hsl_port *port) {
    if (port->host_mdm_wake_lock) {
        pr_debug("[msm_serial_hs]%s\n", __FUNCTION__);
	 //printk("[msm_serial_hs]%s,port->mdm_wake_locked %d\n", __FUNCTION__,port->mdm_wake_locked.counter);
        if (!atomic_cmpxchg(&(port->mdm_wake_locked), 0, 1))//{
            wake_lock_mdm();
	     //printk("[After msm_serial_hs]%s,port->mdm_wake_locked %d\n", __FUNCTION__,port->mdm_wake_locked.counter);}
    }
}

static void mdm_wake_unlock(struct msm_hsl_port *port) {
    if (port->host_mdm_wake_lock) {
        pr_debug("[msm_serial_hs]%s\n", __FUNCTION__);
	 //printk("[msm_serial_hs]%s,port->mdm_wake_locked %d\n", __FUNCTION__,port->mdm_wake_locked.counter);

        if (atomic_cmpxchg(&(port->mdm_wake_locked), 1, 0))//{
            wake_unlock_mdm();
	     //printk("[After msm_serial_hs]%s,port->mdm_wake_locked %d\n", __FUNCTION__,port->mdm_wake_locked.counter);}
    }
}

static void start_mdm_wake_unlock_timer(struct msm_hsl_port *port) {
    if (port->host_mdm_wake_lock) {
        mod_timer(&(port->mdm_wake_unlock_timer), jiffies + HOST_MDM_WAKE_LOCK_TIME_OUT);
    }
}

static void stop_mdm_wake_unlock_timer(struct msm_hsl_port *port) {
    if (port->host_mdm_wake_lock) {
        del_timer(&(port->mdm_wake_unlock_timer));
    }
}

static void mdm_wake_unlock_timer_func(unsigned long data)
{
    struct msm_hsl_port *port = (struct msm_hsl_port*)data;
    //printk("%s call mdm_wake_unlock\n",__FUNCTION__);
    mdm_wake_unlock(port);
}
...
#if 1//def CONFIG_SERIAL_SC6500_WAKE_LOCK
	/*zengwen1, 20130515, modify, just for uart*/
	if(port->line == 1)
	{
		//printk("msm_hsl_stop_tx, call start_mdm_wake_unlock_timer\n");
		start_mdm_wake_unlock_timer(msm_hsl_port);
	}
#endif	
...
static void handle_rx(struct uart_port *port, unsigned int misr)
{
	...
	if(port->line == 1)
	{
		//printk("enter handle_rx stop_mdm_wake_unlock_timer\n");
		stop_mdm_wake_unlock_timer(msm_hsl_port);
		mdm_wake_lock(msm_hsl_port);
	}
#endif
	...
	if(port->line == 1)
	{
		//printk("handle_rx, call start_mdm_wake_unlock_timer\n");
		start_mdm_wake_unlock_timer(msm_hsl_port);
		host_wake_lock(msm_hsl_port);
	}
#endif	   
...
static void handle_tx(struct uart_port *port)
{
	...
	{	
		struct msm_hsl_port *msm_hsl_port = UART_TO_MSM(port);
		//printk("enter handle_tx stop_mdm_wake_unlock_timer\n");
		stop_mdm_wake_unlock_timer(msm_hsl_port);
		mdm_wake_lock(msm_hsl_port);
		host_wake_lock(msm_hsl_port);
...
static void msm_hsl_shutdown(struct uart_port *port)
{
	...
	if(port->line == 1)
	{
		//printk("msm_hsl_shutdown, call start_mdm_wake_unlock_timer\n");
		start_mdm_wake_unlock_timer(msm_hsl_port);
	}
#endif	
...
static int __devinit msm_serial_hsl_probe(struct platform_device *pdev)
{
	...
	if(port->line == 1)
	{
		...
		if(true/*platform_ver == PLATFORM_VERSION_PVT*/){
			msm_hsl_port->host_mdm_wake_lock = true;//pdata->host_mdm_wake_lock;
		}
		else{
			msm_hsl_port->host_mdm_wake_lock = false;
		}
		
		if (msm_hsl_port->host_mdm_wake_lock) {
			init_timer(&msm_hsl_port->mdm_wake_unlock_timer);
			msm_hsl_port->mdm_wake_unlock_timer.function = mdm_wake_unlock_timer_func;
			msm_hsl_port->mdm_wake_unlock_timer.data = (unsigned long)msm_hsl_port;
			atomic_set(&(msm_hsl_port->mdm_wake_locked), 0);
			wake_lock_init(&(msm_hsl_port->host_wake_lock), WAKE_LOCK_SUSPEND, msm_hsl_port->name);
			ret = device_create_file(&(pdev->dev), &dev_attr_host_mdm_wake_lock);
		}
	}
#endif
...
static int __devexit msm_serial_hsl_remove(struct platform_device *pdev)
{
	...
	{
		stop_mdm_wake_unlock_timer(msm_hsl_port);
		//printk("%s call mdm_wake_unlock",__FUNCTION__);
		mdm_wake_unlock(msm_hsl_port);
		host_wake_unlock(msm_hsl_port);
	}
#endif
...
{
	struct platform_device *pdev = to_platform_device(dev);
	struct msm_hsl_port *msm_port;  //zengwen1, 20130515, add
	struct uart_port *port;
	port = get_port_from_line(get_line(pdev));
	msm_port = UART_TO_MSM(port); //zengwen1, 20130515, add
	if (port) {
	...
	{	
		//printk("%s call mdm_wake_unlock",__FUNCTION__);
		stop_mdm_wake_unlock_timer(msm_port);
		mdm_wake_unlock(msm_port);
	}
#endif
	}
