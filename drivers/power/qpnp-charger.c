...
#define VREG_XOADC_MASK			0x03
#define COMP1_OVR_TO_1			0x03
#define COMP1_OVR_TO_0			0x02
#define COMP1_NO_OVR			0x00
#define SEC_ACCESS_VAL			0xA5
...
{
	return qpnp_chg_is_usb_chg_plugged_in(tmp_chip);
}
...
qpnp_chg_usb_chg_gone_irq_handler(int irq, void *_chip)
{
	...
	u8 usb_sts;
	int rc;
   printk("chg_gone triggered, release and rerequest a 3s timeout the temerpature monitor wake lock \n");
   ...
   wake_lock_timeout(&chip->temp_mon_wake_lock,msecs_to_jiffies(CHG_GONE_TIME_OUT_MS));
	...
	schedule_delayed_work(&chip->usb_present_work,
...
static void usb_present_work_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, usb_present_work);
	...
	if (chip->usb_present ^ usb_present) {
		...
		printk("%s usb_present is %d,chip->usb_present is %d\n",__func__,usb_present,chip->usb_present);
		if (chip->usb_present ^ usb_present) {
				chip->usb_present = usb_present;
				if (!usb_present) {
...
qpnp_chg_bat_if_batt_temp_irq_handler(int irq, void *_chip)
{
	...
	__cancel_delayed_work(&chip->temp_mon_work);
...
qpnp_chg_chgr_chg_fastchg_irq_handler(int irq, void *_chip)
{
	...
	printk("FAST_CHG IRQ triggered,request wake lock for temperature monitor\n");
...
extern bool full_capacity;
static int
get_prop_batt_status(struct qpnp_chg_chip *chip)
{
	int rc;
	u8 chgr_sts, bat_if_sts;
	int batt_temp_good =  get_prop_batt_health(chip);
	int hostmode = qpnp_chg_is_otg_en_set(chip);
	if(!get_prop_batt_present(chip))
		return POWER_SUPPLY_STATUS_DISCHARGING;
	if (((qpnp_chg_is_usb_chg_plugged_in(chip) && hostmode == 0) ||
		qpnp_chg_is_dc_chg_plugged_in(chip)) && (chip->chg_done || full_capacity)) {
	...
	if(use_hw_stop_chg == true && qpnp_chg_is_usb_chg_plugged_in(chip) && batt_temp_good == POWER_SUPPLY_HEALTH_GOOD && (hostmode == 0))
		return POWER_SUPPLY_STATUS_CHARGING;
...
static int
get_prop_chg_current(struct qpnp_chg_chip *chip)
{
	union power_supply_propval ret = {0,};
	int current_now = 0;
	...
	if (chip->bms_psy) {
		...
		current_now = ret.intval/1000;
//		printk("chg current is %d\n",current_now);
		if(current_now > 0)
			return 0;
		else
...
get_prop_capacity(struct qpnp_chg_chip *chip)
{
	...
	if (chip->bms_psy) {
		...
		if ((qpnp_chg_is_usb_chg_plugged_in(chip) ||
			qpnp_chg_is_dc_chg_plugged_in(chip)) && (chip->chg_done && !full_capacity)) {
				printk("charging done, but capacity %d is not 100,full_capacity is %d\n",ret.intval,full_capacity);
				return 100;
		}
		last_capacity = soc;		
		return soc;
...
get_prop_batt_temp(struct qpnp_chg_chip *chip)
{
	...
	if(use_hw_stop_chg == true && batt_temp_good == POWER_SUPPLY_HEALTH_GOOD &&  (results.physical > 520 || results.physical < 0))
	{
		printk("adc read value may be error %lld\n",results.physical);
		return last_temp;
	}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHG_CURRENT:
		val->intval = get_prop_chg_current(chip);
		break;
	default:
		return -EINVAL;
	}
...
qpnp_temp_mon_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, temp_mon_work);
	int temp = 0;
	int delay_ms = 0;
	int current_now = 0;
	int rc = 0;
	...
	//when the pmic stop the charging automatically we close this feature and start software check.
		if(use_hw_stop_chg == true)
		{
			rc = qpnp_chg_masked_write(chip,
				...
				2, 1);
			if (rc) {
				pr_debug("failed to chose BTC rc=%d\n", rc);
				return ;
			}
	...
	printk("battery temp is %d,tempabnormal is %d,use_hw_stop_chg is %d,current now is %d\n",(int)temp,(int)temp_abnormal,(int)use_hw_stop_chg,current_now);

	if(chip->bat_is_cool ==true && temp > 0 && batt_voltage > 4050 && dec_cur_bat_cool == false)
	{
		printk("%s:decrease charging current\n",__func__);
		dec_cur_bat_cool = true;
//		qpnp_chg_vddmax_set(chip, 4350);
		qpnp_chg_ibatmax_set(chip, 300);
//		qpnp_chg_vbatdet_set(chip, 4250);
	}

	if(dec_cur_bat_cool == true && (batt_voltage <= 4050 || (chip->bat_is_cool == false && temp > 0)))
	{
		printk("%s:resume charging current\n",__func__);
...
//		qpnp_chg_set_appropriate_vbatdet(chip);
	...
	if(chip->cool_bat_decidegc < temp && temp < chip->warm_bat_decidegc)
		delay_ms = 60*3*1000;
	else
		delay_ms = 30*1000;
	if(wake_lock_active(&chip->temp_mon_wake_lock) || temp_abnormal == true)
				...
				struct spmi_resource *spmi_resource)
{
	...
	case SMBBP_BAT_IF_SUBTYPE:
	case SMBCL_BAT_IF_SUBTYPE:
        printk("%s:keep BATFET_CP_DRV always 7v when sleep.\n",__func__);
        rc = qpnp_chg_masked_write(chip,
                chip->bat_if_base+0xdf,
                0xC0,      //just write bit[6:7]
                0x00,1);
		if (rc) {
			pr_info("failed to set BATFET_CP_DRV enable, rc=%d\n", rc);
		}
	...
	case SMBB_DC_CHGPTH_SUBTYPE:
		rc = qpnp_chg_masked_write(chip,
			chip->dc_chgpth_base + DC_OVP_CTL,
			0x30,
			0x0, 1);
		break;
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->usb_present_work, usb_present_work_func);
...
static void qpnp_charger_shutdown(struct spmi_device *dev)
{
	struct qpnp_chg_chip *chip = dev_get_drvdata(&(dev->dev));

	if (!qpnp_chg_is_batt_present(chip))
	{
		qpnp_chg_masked_write(chip, chip->chgr_base + CHGR_CHG_CTRL,
			CHGR_CHG_EN|CHGR_ON_BAT_FORCE_BIT,
			0, 1);
		printk("enable VBUS power supply even battery is absent during shutdown\n");
	}
}
...
int dump_chg_reg = 0;
static int dump_chg_reg_func(const char *s, struct kernel_param *kp)
{
	int i;
	u8 reg_data = 0;
	if(!strncmp(s,"1",1))
	{
		printk("\n*************************beging dump chg register*****************************\n");
		 for (i = 0; i < 0x700; i++)
		 {
			qpnp_chg_read(tmp_chip, &reg_data,0x1000+i, 1);
			printk("%x: %02x\t",0x1000+i,reg_data);
			if(i%10 == 9)
				printk("\n");
		 
		 }
		printk("\n*************************dump chg register finished*****************************\n");
			
	}
			
	return 0;
}
module_param_call(dump_chg_reg,dump_chg_reg_func,NULL,NULL,0644);
/**
