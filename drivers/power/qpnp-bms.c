...
static int get_battery_voltage_now(struct qpnp_bms_chip *chip)
{
	union power_supply_propval ret = {0,};
...
static int report_cc_based_soc(struct qpnp_bms_chip *chip)
{
	...
	if (rc) {
		...
		return chip->last_soc;//return last soc when read adc failed
		//	return rc;
	}
	...
	if(low_vol_count > 5 && soc > 3 && soc < 15)
	{
		soc_change = soc;
		soc = 0;
	}
							...
							int soc, int batt_temp)
{
	...
	int battery_voltage = get_battery_voltage_now(chip)/1000;
	...
	if(ocv_est_uv < 3450000 && battery_voltage < 3450)
	{
		low_vol_count++;
		printk("OCV est %d and battery voltage %d below 3450mV\n",ocv_est_uv,battery_voltage);
	}
	else
		low_vol_count = 0;
...
static void calculate_soc_work(struct work_struct *work)
{
	...
	int battery_voltage = get_battery_voltage_now(chip)/1000;
	int batt_temp = get_battery_temp(chip);
	...
	printk("Battery capacity is %d,voltage is %d,temperature is %d,status is %d,current is %d,charger counter is %d\n",soc,battery_voltage,batt_temp,get_battery_status(chip),get_prop_bms_current_now(chip),get_prop_bms_charge_counter(chip));
}
	...
	if (chip->battery_present != present
		...
		if (chip->battery_present != -EINVAL) {
			if (present) {
				...
				//there is vbat_monitor_mutex in reset_vbat_monioring,so unlock it here first 
				mutex_unlock(&chip->vbat_monitor_mutex);
...
extern struct bms_battery_data Lenovo_Kiton_3000mAh_data;
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	} else if (chip->batt_type == BATT_LENOVO_KITON) {
		batt_data = &Lenovo_Kiton_3000mAh_data;
	} else {
