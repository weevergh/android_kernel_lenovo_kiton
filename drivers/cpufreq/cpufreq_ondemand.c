...
static void dbs_freq_increase(struct cpufreq_policy *p, unsigned int freq)
{
	...
	__cpufreq_driver_target(p, freq, CPUFREQ_RELATION_L); //dbs_tuners_ins.powersave_bias ?
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
    ...
    step = policy->max * 3 / 10;
    start_freq = policy->max / 2;
    target = policy->cur;
    while(max_load_freq > dbs_tuners_ins.up_threshold * target)
    {
        if(target < start_freq)
            target = start_freq;
        else
            target += step;
        found = 1;
	...
	if (max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) {
        unsigned target = policy->max * 50 / 100;
        ...
        if(policy->cur > target)
            target = policy->cur + policy->max * 20 / 100;
		dbs_freq_increase(policy, target);
		return;
	}
#endif
