...
extern int mdss_set_gamma_mode(struct msm_fb_data_type *mfd, void __user * p);
extern int mdss_set_ce_mode(struct msm_fb_data_type *mfd, void __user * p);
extern int mdss_set_sharpness_mode(struct msm_fb_data_type *mfd, void __user * p);
extern int mdss_set_cabc_mode(struct msm_fb_data_type *mfd, void __user * p);
extern int mdss_set_final_vcom(struct msm_fb_data_type *mfd, void __user * p);
...
extern int LCD_ID;
static ssize_t mdss_lcd_get_name(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	//struct fb_info *fbi = dev_get_drvdata(dev);
	//struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	...
	switch (LCD_ID) {
	case LG_R63311_1080P_VID:
		ret = snprintf(buf, PAGE_SIZE, "LG_R63311\n");
		break;
	case YAS_NT35595_1080P_VID:
		ret = snprintf(buf, PAGE_SIZE, "YAS_NT35595\n");
		break;
	case YAS_NT35596_1080P_VID:
		ret = snprintf(buf, PAGE_SIZE, "YAS_NT35595\n");
		break;
	default:
		ret = snprintf(buf, PAGE_SIZE, "unknown panel\n");
...
extern ssize_t show_dsi_register_value(char * buf);
static ssize_t mdss_show_dsi_register(struct device *dev, struct device_attribute *attr, char *buf)
{
	return show_dsi_register_value(buf);
}
...
static DEVICE_ATTR(lcd_name, S_IRUGO | S_IWUSR | S_IXUSR, mdss_lcd_get_name, NULL);
static DEVICE_ATTR(show_dsi_register, S_IRUGO | S_IWUSR | S_IXUSR, mdss_show_dsi_register, NULL);
...
static struct attribute *mdss_fb_attrs[] = {
	...
	&dev_attr_lcd_name.attr,
       &dev_attr_show_dsi_register.attr,
	NULL,
};
			 ...
			 unsigned long arg)
{
	...
	case MSMFB_DISPLAY_COMMIT:
		...
		break;
	case MSMFB_SET_GAMMA_MODE:
		ret = mdss_set_gamma_mode(mfd, argp);
		break;

	case MSMFB_SET_CE_MODE:
		ret = mdss_set_ce_mode(mfd, argp);
		break;
	case MSMFB_SET_SHARPNESS_MODE:
		ret = mdss_set_sharpness_mode(mfd, argp);
		break;
	case MSMFB_SET_CABC_MODE:
		ret = mdss_set_cabc_mode(mfd, argp);
		break;
	case MSMFB_SET_FINAL_VCOM:
		ret = mdss_set_final_vcom(mfd, argp);
		break;
	default:
