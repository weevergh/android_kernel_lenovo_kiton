...
//#ifdef LCD_DEBUG
static unsigned char *msm_mmss_clk_base = NULL;
static unsigned char *mipi_dsi_base1 = NULL;
#define MSM_MMSS_CLK_CTL_BASE1       0xFD8C0000
#define MSM_MMSS_CLK_SIZE                    0x3000
#define DSI_PHY_PHYS1		0xFD922800
#define DSI_PHY_SIZE1		0x00000800
void show_dbg_register_value(void)
{
	uint16_t data;
	int i = 0;
	int j = 0;
	...
	if(!mipi_dsi_base1)
		mipi_dsi_base1 = ioremap(DSI_PHY_PHYS1, DSI_PHY_SIZE1);
	if (!mipi_dsi_base1)
		pr_err("%s: unable to ioremap dsi base", __func__);
	for(i=0; i<114; i++)
	{
		printk("%04x: ",i*16);
		for(j=0; j<4; j++)
			printk("%08x ",MIPI_INP((mipi_dsi_base1) + i*4*4+j*4));
		printk("\n");
	}

	if(!msm_mmss_clk_base)
		msm_mmss_clk_base = ioremap(MSM_MMSS_CLK_CTL_BASE1, MSM_MMSS_CLK_SIZE);
	if (!msm_mmss_clk_base)
		pr_err("%s: unable to ioremap clk phy pll base", __func__);
	
	data = readl_relaxed(msm_mmss_clk_base + 0x2124);
	printk("%s: DSI_BYTE0_CFG_RCGR data = 0x%x\n",__func__, data);

	data = readl_relaxed(msm_mmss_clk_base + 0x2120);
	printk("%s: DSI_BYTE0_CMD_RCGR data = 0x%x\n",__func__, data);
		
	data = readl_relaxed(msm_mmss_clk_base + 0x233C);
	printk("%s: DSI_BYTE0_CBCR data = 0x%x\n",__func__, data);

	data = readl_relaxed(msm_mmss_clk_base + 0x2004);
	printk("%s: DSI_PIXEL0_CFG_RCGR data = 0x%x\n",__func__, data);
	
	data = readl_relaxed(msm_mmss_clk_base + 0x2000);
	printk("%s: DSI_PIXEL0_CMD_RCGR data = 0x%x\n",__func__, data);
		
	data = readl_relaxed(msm_mmss_clk_base + 0x2314);
	printk("%s: DSI_PIXEL0_CBCR data = 0x%x\n\n",__func__, data);

	data = readl_relaxed(msm_mmss_clk_base + 0x2160);
	printk("%s: DSI_ESC0_CMD_RCGR data = 0x%x\n",__func__, data);
	
	data = readl_relaxed(msm_mmss_clk_base + 0x2164);
	printk("%s: DSI_ESC0_CFG_RCGR data = 0x%x\n",__func__, data);
		
	data = readl_relaxed(msm_mmss_clk_base + 0x2344);
	printk("%s: DSI_ESC0_CBCR data = 0x%x\n\n",__func__, data);


	data = readl_relaxed(msm_mmss_clk_base + 0x2308);
	printk("%s: MDP_AHB_CBCR data = 0x%x\n",__func__, data);
	
	data = readl_relaxed(msm_mmss_clk_base + 0x2040);
	printk("%s: MDP_CMD_RCGR data = 0x%x\n",__func__, data);

	data = readl_relaxed(msm_mmss_clk_base + 0x2044);
	printk("%s: MDP_CFG_RCGR data = 0x%x\n",__func__, data);
	
	data = readl_relaxed(msm_mmss_clk_base + 0x231C);
	printk("%s: MDP_CBCR data = 0x%x\n",__func__, data);
	
	data = readl_relaxed(msm_mmss_clk_base + 0x2320);
	printk("%s: MDP_LUT_CBCR data = 0x%x\n",__func__, data);
}
//#endif

struct mdss_panel_data *g_pdata = NULL;
ssize_t show_dsi_register_value(char * buf)
{
	char * ptr = buf;
	uint16_t data;
	int i = 0;
	...
	if(g_pdata->panel_info.panel_power_on == 0)
	{
		ptr += sprintf(ptr,"dsi is power off\n");
		return ptr - buf;
	}
	...
	if(bswitch)
	{
		for(i=0; i<70; i++)//114
		{
			ptr += sprintf(ptr, "%03x: ", i*16);
			for(j=0; j<4; j++)
				ptr += sprintf(ptr, "%08x ",MIPI_INP((mipi_dsi_base1) + i*4*4+j*4));
			ptr += sprintf(ptr,"\n");
		}
	}else
	{
		for(i=70; i<114; i++)//114
		{
			ptr += sprintf(ptr, "%03x: ", i*16);
			for(j=0; j<4; j++)
				ptr += sprintf(ptr, "%08x ",MIPI_INP((mipi_dsi_base1) + i*4*4+j*4));
		...
		data = readl_relaxed(msm_mmss_clk_base + 0x2124);
		ptr += sprintf(ptr, "DSI_BYTE0_CFG_RCGR=0x%x\n", data);

		data = readl_relaxed(msm_mmss_clk_base + 0x2120);
		ptr += sprintf(ptr, "DSI_BYTE0_CMD_RCGR=0x%x\n", data);
			
		data = readl_relaxed(msm_mmss_clk_base + 0x233C);
		ptr += sprintf(ptr, "DSI_BYTE0_CBCR=0x%x\n", data);

		data = readl_relaxed(msm_mmss_clk_base + 0x2004);
		ptr += sprintf(ptr, "DSI_PIXEL0_CFG_RCGR=0x%x\n", data);
		
		data = readl_relaxed(msm_mmss_clk_base + 0x2000);
		ptr += sprintf(ptr, "DSI_PIXEL0_CMD_RCGR=0x%x\n", data);
			
		data = readl_relaxed(msm_mmss_clk_base + 0x2314);
		ptr += sprintf(ptr, "DSI_PIXEL0_CBCR=0x%x\n", data);

		data = readl_relaxed(msm_mmss_clk_base + 0x2160);
		ptr += sprintf(ptr, "DSI_ESC0_CMD_RCGR=0x%x\n", data);
		
		data = readl_relaxed(msm_mmss_clk_base + 0x2164);
		ptr += sprintf(ptr, "DSI_ESC0_CFG_RCGR=0x%x\n", data);
			
		data = readl_relaxed(msm_mmss_clk_base + 0x2344);
		ptr += sprintf(ptr, "DSI_ESC0_CBCR=0x%x\n", data);

		data = readl_relaxed(msm_mmss_clk_base + 0x2308);
		ptr += sprintf(ptr, "MDP_AHB_CBCR=0x%x\n", data);
		
		data = readl_relaxed(msm_mmss_clk_base + 0x2040);
		ptr += sprintf(ptr, "MDP_CMD__RCGR=0x%x\n", data);

		data = readl_relaxed(msm_mmss_clk_base + 0x2044);
		ptr += sprintf(ptr, "MDP_CFG_RCGR=0x%x\n", data);
		
		data = readl_relaxed(msm_mmss_clk_base + 0x231C);
		ptr += sprintf(ptr, "MDP_CBCR=0x%x\n", data);
		
		data = readl_relaxed(msm_mmss_clk_base + 0x2320);
		ptr += sprintf(ptr, "MDP_LUT_CBCR=0x%x\n", data);
	}
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (ret) {
			...
			gpio_direction_output(g_vsp_en, 1);
			udelay(100);
			wmb();
			gpio_direction_output(g_vsn_en, 1);
			msleep(4);
			wmb();

	} else {

		gpio_direction_output(g_vsn_en, 0);
		udelay(100);
		wmb();
		gpio_direction_output(g_vsp_en, 0);
		msleep(10);
		wmb();
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	g_pdata = pdata;
	pr_info("[DIS] %s: cont_splash_enabled=%d##\n", __func__, pdata->panel_info.cont_splash_enabled);
	if (pdata->panel_info.panel_power_on) {
	...
	//if(first_blacklight == 0){
       //       ctrl_pdata->panel_data.set_backlight(pdata,1000);
	 //       first_blacklight = 1;
	//}
	pr_debug("%s-:\n", __func__);
	return 0;
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_UNBLANK:
		printk("[LCD] %s :MDSS_EVENT_UNBLANK\n", __func__);
	...
	case MDSS_EVENT_PANEL_ON:
		printk("[LCD] %s :MDSS_EVENT_PANEL_ON\n", __func__);
		ctrl_pdata->ctrl_state |= CTRL_STATE_MDP_ACTIVE;
		if (ctrl_pdata->on_cmds.link_state == DSI_HS_MODE)
			rc = mdss_dsi_unblank(pdata);
		break;
	case MDSS_EVENT_BLANK:
		printk("[LCD] %s :MDSS_EVENT_BLANK\n", __func__);
		if (ctrl_pdata->off_cmds.link_state == DSI_HS_MODE)
			rc = mdss_dsi_blank(pdata);
		break;
	case MDSS_EVENT_PANEL_OFF:
		printk("[LCD] %s :MDSS_EVENT_PANEL_OFF\n", __func__);
		ctrl_pdata->ctrl_state &= ~CTRL_STATE_MDP_ACTIVE;
	...
	case MDSS_EVENT_CONT_SPLASH_FINISH:
		printk("[LCD] %s :MDSS_EVENT_CONT_SPLASH_FINISH\n", __func__);
		ctrl_pdata->ctrl_state &= ~CTRL_STATE_MDP_ACTIVE;
		if (ctrl_pdata->on_cmds.link_state == DSI_LP_MODE) {
	...
	case MDSS_EVENT_PANEL_CLK_CTRL:
		printk("[LCD] %s :MDSS_EVENT_PANEL_CLK_CTRL\n", __func__);
		mdss_dsi_clk_req(ctrl_pdata, (int)arg);
		break;
	case MDSS_EVENT_DSI_CMDLIST_KOFF:
		printk("[LCD] %s :MDSS_EVENT_DSI_CMDLIST_KOFF\n", __func__);
		ctrl_pdata->recovery = (struct mdss_panel_recovery *)arg;
		mdss_dsi_cmdlist_commit(ctrl_pdata, 1);
		break;
	case MDSS_EVENT_PANEL_UPDATE_FPS:
		printk("[LCD] %s :MDSS_EVENT_PANEL_UPDATE_FPS\n", __func__);
		if (arg != NULL) {
	...
	case MDSS_EVENT_CONT_SPLASH_BEGIN:
		printk("[LCD] %s :MDSS_EVENT_CONT_SPLASH_BEGIN\n", __func__);
		if (ctrl_pdata->off_cmds.link_state == DSI_HS_MODE) {
	...
	case MDSS_EVENT_ENABLE_PARTIAL_UPDATE:
		printk("[LCD] %s :MDSS_EVENT_ENABLE_PARTIAL_UPDATE\n", __func__);
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	*/
	...
	g_vsp_en = of_get_named_gpio(ctrl_pdev->dev.of_node, "qcom,vsp-enable-gpio", 0);
	//g_vsp_en = 16;
	printk(" ***************** g_vsp_en = %d **********************\n", g_vsp_en);
	if (!gpio_is_valid(g_vsp_en)) {
		pr_err("%s:%d, g_vsp_en gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(g_vsp_en, "vsp_en_n");
		if (rc) {
			pr_err("request vsp_en gpio failed, rc=%d\n",
				rc);
			gpio_free(g_vsp_en);
			gpio_free(ctrl_pdata->rst_gpio);
			//gpio_free(ctrl_pdata->disp_en_gpio);
	...
	g_vsn_en = of_get_named_gpio(ctrl_pdev->dev.of_node, "qcom,vsn-enable-gpio", 0);
	//g_vsn_en = 14;
	printk(" ***************** g_vsn_en = %d **********************\n", g_vsn_en);
	if (!gpio_is_valid(g_vsn_en)) {
		pr_err("%s:%d, g_vsn_en gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(g_vsn_en, "vsn_en_n");
		if (rc) {
			pr_err("request g_vsn_en gpio failed, rc=%d\n",
				rc);
			gpio_free(g_vsn_en);
			gpio_free(g_vsp_en);
			gpio_free(ctrl_pdata->rst_gpio);
	...
	printk("[DIS] %s: %d  %d  %d",__func__, ctrl_pdata->rst_gpio, g_vsp_en, g_vsn_en);
